...
DEFAULT_ATTRIBUTES = {"status": BookProgressChoice.UNREAD.value}
...
class TagManager:
    ...
    def create_virtual_tags(self, book_id: int) -> list[VirtualTag]:
        ...
        bookmark = (
            self.db_session.query(Bookmark)
            .filter_by(book_id=book_id, user_id=self.user_id)
            .first()
        )
        if not bookmark:
            return []
    ...
    def get_user_tags(self, book_id: int) -> list[Tag]:
        ...
        return (
            self.db_session.query(Tag.name)
            .join(
                book_tags,
                db.and_(
    ...
    def update_from_virtual_tag(self, book_id: int, field: str, value: str | None):
        ...
        if bookmark is None:
            ...
            logging.info(
                f"Updating bookmark {bookmark} with field {field} and value {value}"
            )
